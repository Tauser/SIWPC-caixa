 ...
 * A sample portlet
 */
public class FaleConoscoPortlet extends javax.portlet.GenericPortlet {
	...
	private static enum NaturezaFormulario{
		...
		DEUNCIAS(4,"Denúncias","denúncia",2),
	...
	@ProcessAction(name="reabrirReclamacao")
	public void reabrirReclamacao (ActionRequest request, ActionResponse response)throws PortletException, IOException{
		try{
			
			FaleConoscoService service = new FaleConoscoService();
			OcorrenciaDTO ocorrenciaSessao = (OcorrenciaDTO) request.getPortletSession().getAttribute(OCORRENCIA);
			OcorrenciaDTO dto = service.buscaReclamacao(ocorrenciaSessao.getProtocolo().toString());
			String msg = request.getParameter("mensagem");
			String categoria = request.getParameter("categoria");
			msg += "\nProtocolo anterior: "+dto.getProtocolo();
			dto.setTipo(2);//o tipo de ocorrencia altera para o tipo Ouvidoria
			dto.setCategoria(categoria);
			dto.setMensagem(msg);
			Integer protocolo = service.gravaFormulario(dto);
			response.setRenderParameter(PROTOCOLO, protocolo.toString());
			request.getPortletSession().removeAttribute(OCORRENCIA);
		}catch(Exception e){
			e.printStackTrace();
	...
	public void gravar (ActionRequest request, ActionResponse response)throws PortletException, IOException{
		try{
			...
			}else if(natureza.equalsIgnoreCase(Integer.toString(NaturezaFormulario.DEUNCIAS.natureza))){
				//recuperar o nome da categoria de denuncia e adiciona ao Manifesto
				String categoriaDeDenuncia = service.getNomeCategoriaDenuncia(dto.getCategoria());
				
				
				//atribuio o valor da categoria para a categoria Outros Servicos do SIOUV
				dto.setCategoria(CATEGORIA_SIOUV_OUTROS_SERVICOS);
				
				//incluir o nome da categoria no manifesto
				if(!categoriaDeDenuncia.isEmpty()){
					String mensagem = dto.getMensagem();
					dto.setMensagem("CATEGORIA DA DENÚNCIA: "+categoriaDeDenuncia.concat("\n").concat(mensagem));
				}
	...
	private String getTituloForm(int tipo) {
		...
		}else if(tipo == NaturezaFormulario.DEUNCIAS.natureza){
			titulo = NaturezaFormulario.DEUNCIAS.titulo;
	...
	private int getTipoOCorrecia(int tipo) {
		...
		if(tipo == NaturezaFormulario.SUGESTOES.natureza){
			...
			tipoOcorrencia = NaturezaFormulario.DEUNCIAS.tipoOcorrencia;
	...
	private String getNomeNatureza(int natureza) {
		...
		}else if(natureza == NaturezaFormulario.DEUNCIAS.natureza){
			nome = NaturezaFormulario.DEUNCIAS.nome;
